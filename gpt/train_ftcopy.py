...
Bt_param = [value for param, value in model.named_parameters() if 'bias' in param]
optim = AdamW(params=Bt_param, lr=lr)
...
model.train()
param = count_param(model, param_name='bias')
file_path = f'./save/{task}/{plm}/Bt_lr_{lr}_line_{epoch}_bsz_{bsz}_step_{rec_step}_{param}'
...
save_path = './save/Bt_prediction/' + plm
os.makedirs(save_path) if not os.path.exists(save_path) else None
dic = torch.load(file_path+f'/Bt_{task}best_ec_param.pt')
...
#     file_path = '/data1/zwx/ex_experiment/gpt/save/xsum/gpt2-large/Bt_lr_0.0001_line_1_bsz_16_step_150_time_52976.208089113235'
