...
def train_ft(task, seed, class_num, bsz, lr, epoch, plm, tag, rec_step):
    config = RobertaConfig.from_pretrained(plm)
    config.lr = lr
    config.epoch = epoch
    config.num_labels = class_num
    file_path = f'./save/{task}/{plm}/Ft_lr_{lr}_line_{epoch}_bsz_{bsz}_step_{rec_step}_seed_{seed}'
    ...
    save_path = './save/Ft_prediction/' + plm + f'/seed_{seed}'
