...
def train_bt(task, seed, class_num, bsz, lr, epoch, plm, tag, rec_step, data_size):
    ...
    model.train()
    param = count_param(model=model, param_name='bias')
    file_path = f'./save/few_shot/{task}/seed_{seed}/{plm}/Bt_lr_{lr}_line_{epoch}_bsz_{bsz}_step_{rec_step}_{param}'
    ...
    dic = torch.load(file_path + f'/Bt_{task}best_acc_param.pt')
    ...
    with open(save_path + f'/Bt_report.txt', 'a+', encoding='utf-8') as f:
        f.write(report)
    return sum_p
