...
from transformers.modeling_outputs import SequenceClassifierOutput, BaseModelOutput, TokenClassifierOutput
...
class RobertaPrefixForSequenceClassification(RobertaPreTrainedModel):
    def __init__(self, config, plm):
        ...
        self.num_labels = config.num_labels
        self.config = config
        self.roberta = RobertaModel.from_pretrained(plm)
        self.dropout = torch.nn.Dropout(config.hidden_dropout_prob)
        self.classifier = torch.nn.Linear(config.hidden_size, config.num_labels)
        # self.init_weights()
    ...
    def load(self, save_path):
        ...
        state_dict = torch.load(save_path + 'classifier.pt', map_location=self.device)
