...
def train_aug(task, seed, class_num, bsz, lr, epoch, plm, add_dim, tag, rec_step, line_m):
    ...
    file_path = f'./save/{task}/{plm}/Augment_{add_dim}_lr_{lr}_line_{epoch}_bsz_{bsz}_step_{rec_step}_seed_{seed}'
    ...
    save_path = './save/Aug_prediction/' + plm + f'/seed_{seed}'
    ...
    torch.save(pre, save_path + f'/{task}_prediction_ac_{add_dim}.pt')
