...
def train_prompt(task, seed, class_num, bsz, lr, epoch, plm, tag, rcs, pre_seq_len):
    file_path = f'./save/{task}/{plm}/Prompt_tok_{pre_seq_len}_lr_{lr}_line_{epoch}_seed_{seed}'
    ...
    # metric = load_metric('glue', f'{task}')
    ...
    scheduler = get_linear_schedule_with_warmup(optim, num_warmup_steps=config.epoch * train_data.batch_num // 3,
                                                num_training_steps=config.epoch * train_data.batch_num)
    model.train()
    rec.start()
    ...
    save_path = './save/Prompt_prediction/' + plm + f'/seed_{seed}'
    ...
    with torch.no_grad():
        ...
        for test_step, test_batch in enumerate(test_data):
            ...
            pre += output.logits.argmax(dim=-1).view(-1).tolist()
    torch.save(pre, save_path + f'/{task}_prediction_ac.pt')
