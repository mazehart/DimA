...
def train_lora(task, seed, class_num, bsz, lr, epoch, plm, tag, rec_step, type_m):
    ...
    config.adapter_size = 16 
    config.epoch = epoch
    config.lr = lr
    config.num_labels = class_num
    file_path = f'./save/{task}/{plm}/{type_m}_lr_{lr}_line_{epoch}_bsz_{bsz}_step_{rec_step}_seed_{seed}'
    ...
    save_path = f'./save/{type_m}_prediction/' + plm + f'/seed_{seed}'
